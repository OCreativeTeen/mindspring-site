...
按顺序做完下面每一步，即可用 **mindstoryroom.com** 访问你的网站。
...
## 第三步：在 Vercel 里添加域名 mindstoryroom.com
...
1. 在 **Domain** 输入框里输入：**mindstoryroom.com**，点 **Add**。
2. 再添加一条：**www.mindstoryroom.com**（可选，建议加）。
3. 添加后 Vercel 会显示 “Invalid Configuration” 或 “未配置”，并给出需要填的 DNS 记录，这是正常的，下一步去 BlueHost 配置即可。

### 3.3 记下 Vercel 要求的 DNS（备用）

- 根域名 **mindstoryroom.com**：通常是 **A 记录** 指向 **76.76.21.21**，或 CNAME 指向 **cname.vercel-dns.com**（以 Vercel 页面显示为准）。
...
2. 进入 **My Sites** 或 **Domains**，找到 **mindstoryroom.com**。
3. 点击该域名旁的 **Manage** 或 **DNS** / **Zone Editor**（名称可能是「DNS 设置」「Manage DNS」「Zone File」）。
4. 若在 cPanel：**Domains** → **Zone Editor**，选中 **mindstoryroom.com**。

### 4.2 配置根域名（mindstoryroom.com）

- 找到 **Type** 为 **A**、**Name** 为 **@** 或 **mindstoryroom.com** 的那一条。
- 点击 **Edit**（编辑）：
...
### 4.3 配置 www（www.mindstoryroom.com）
...
2. 刷新页面，等待 **mindstoryroom.com** 和 **www.mindstoryroom.com** 状态变为 **Valid**（或显示证书已签发）。
...
- 浏览器打开：**https://mindstoryroom.com**
- 再试：**https://www.mindstoryroom.com**
...
| 3 | Vercel | Settings → Domains → 添加 mindstoryroom.com 和 www.mindstoryroom.com |
| 4 | BlueHost | DNS：A @ → 76.76.21.21；CNAME www → cname.vercel-dns.com |
| 5 | Vercel | Domains 里等状态变 Valid |
| 6 | 浏览器 | 访问 https://mindstoryroom.com 和 https://www.mindstoryroom.com |
...
**Q：改完 DNS 很久还是打不开？**  
A：用 https://www.whatsmydns.net 查 **mindstoryroom.com** 的 A 记录是否已是 **76.76.21.21**。若已生效但 Vercel 仍显示 Invalid，可等几分钟再刷新 Vercel 的 Domains 页。

**Q：之后更新网站怎么弄？**  
A：在本地改代码后执行：`git add .` → `git commit -m "更新说明"` → `git push`。Vercel 会自动重新部署，mindstoryroom.com 会显示最新版本。

完成以上步骤后，你的网站就通过 BlueHost 的域名 **mindstoryroom.com** 正式上线了。
